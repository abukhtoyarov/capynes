//
// Mapper 23 (Konami VRC2b) - baseline implementation.
//
use super::Mapper;
use super::Mirroring;
...
impl Vrc2b23 {
    ...
    fn chr_bank_value(&self, i: usize) -> usize {
        let raw = ((self.chr_bank_high[i] & 0x0F) << 4) | (self.chr_bank_low[i] & 0x0F);
        (raw as usize) % self.chr_1k_banks()
    }
    ...
    fn decode_chr_reg(addr: u16) -> Option<(usize, bool)> {
        ...
        let reg = base + ((sub >> 1) & 0x01);
        let high = (sub & 0x01) != 0;
        Some((reg, high))
    }
}

impl Mapper for Vrc2b23 {
